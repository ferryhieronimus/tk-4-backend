...
class VBEPostings:
    ...
    def vb_encode_number(number):
        ...
        """
        # TODO
        # referensi: https://nlp.stanford.edu/IR-book/pdf/05comp.pdf
        encoded_bytes = []

        while True:
            encoded_bytes.insert(0, number % 128)
            if number < 128:
                break
            number = number // 128
    ...
    def vb_encode(list_of_numbers):
        ...
        for number in list_of_numbers:
            vb_encoded_number = VBEPostings.vb_encode_number(number)
            encoded_numbers.extend(vb_encoded_number)
    ...
    def encode(postings_list):
        ...
        # TODO
        encoded_list = []
        prev_docID = 0  # initial

        for docID in postings_list:
            gap = docID - prev_docID

            # encode
            vb_encoded_gap = VBEPostings.vb_encode_number(gap)

            # menambahkan hasil encode ke list
            encoded_list.extend(vb_encoded_gap)
            prev_docID = docID

        # mengembalikan dalam bentuk bytes
    ...
    def decode(encoded_postings_list):
        ...
        # TODO
        # referensi: https://nlp.stanford.edu/IR-book/pdf/05comp.pdf
        vb_decoded_list = VBEPostings.vb_decode(encoded_postings_list)
        decoded_list = []
        prev_docID = 0

        for gap in vb_decoded_list:
            docID = prev_docID + gap
            decoded_list.append(docID)
            prev_docID = docID
