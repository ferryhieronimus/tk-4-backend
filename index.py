...
class InvertedIndexReader(InvertedIndex):
    ...
    def get_postings_list(self, term):
        ...
        """
        # TODO
        # cek apakah term berada di postings list
        if term in self.postings_dict:
            start_position, num_postings, postings_length, tf_length = self.postings_dict[
                term]

            # read sepanjang `postings_length`
            self.index_file.seek(start_position)
            postings_list = self.index_file.read(postings_length)
            tf_list = self.index_file.read(postings_length + tf_length)

            # decode postings_list terlebih dahulu
            decoded_postings_list = self.postings_encoding.decode(
                postings_list)

            # decode tf_list terlebih dahulu
            decoded_tf_list = self.postings_encoding.decode_tf(tf_list)
...
class InvertedIndexWriter(InvertedIndex):
    ...
    def append(self, term, postings_list, tf_list):
        ...
        # buat tuple seperti yang diminta
        postings_info = (
            self.index_file.tell(),  # mendapatkan posisi pointer sekarang
            len(postings_list),
            len(encoded_postings_list),
            len(encoded_tf_list)
        )

        tuple = list(zip(postings_list, tf_list))

        # mengupdate doc_length
        for item in tuple:
            key, value = item
            if self.doc_length.get(key) is not None:
                self.doc_length[key] += value
            else:
                self.doc_length[key] = value

        self.terms.append(term)
        self.postings_dict[term] = postings_info
